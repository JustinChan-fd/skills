...
| `boundary_total` | Four-way sum of the **last** API call only | The context footprint at run end (~"how deep in the session was this?"). **Confound #1's measuring stick.** Also the only quantity an observer *outside* the session can see, so it is what an external dispatcher's own token count should be reconciled against. |
