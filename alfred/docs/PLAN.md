...
### 8.3 Fixture in git, or regenerated by script? — DECIDED for both
...
**SANDBOX repo: in git, ONE copy, shared by manifest reference. DECIDED 2026-07-30,
on building `sandbox-b`.** None of the transcript reasoning transfers — the sandbox
holds real content — so this was decided on its own evidence.

The paragraph above weighs a **6.7MB bare repo**, and that framing was the thing
that kept this open. The sandbox is not that: it is a 14-file synthetic service
authored for this purpose — `du -sh fixtures/sandbox-a/files` says **56KB**, with no
privacy question at all,
because every byte was written here. Script-generation is not even an option — there
is no upstream to regenerate from, so "in git" was never really the expensive
choice. The 6.7MB figure belongs to the *TARS-1339 clone*, a separate fixture, and
carrying it into this row made a cheap decision look costly for a year.

What was actually open, once the size question dissolved, is **how many copies**.
`sandbox-b` reuses `sandbox-a`'s tree — SANDBOX.md §7 wants the repo to stop being a
variable so differences are attributable to the ticket. Two ways to honour that:

- **A copy per slug.** Independent, and fails **silently** — edit sandbox-a's
  `sms.js`, sandbox-b's copy does not move, both ground-truth suites stay green, and
  "same repo" degrades into a claim maintained by hand.
- **One tree, shared by reference.** `sandbox-b` carries no `files/`; its manifest
  declares `files_from: "sandbox-a"`. Fails **loudly** — one tree means one set of
  expected shas, so an edit fails every sharing fixture's ground truth at once.

Chose the second. The coupling exists either way; this makes it visible. The cost is
asserted, not hidden: editing `sandbox-a/files/` moves sandbox-b's ground truth too,
and **both** manifests must be re-measured.
`test/fixture-shared-tree.test.mjs` (13 tests) fails if only one is updated and
names the other in its failure message. The resolver refuses both-`files/`-and-
`files_from`, neither, and chains — one hop only, so "which tree did this provision"
never becomes a graph traversal.
